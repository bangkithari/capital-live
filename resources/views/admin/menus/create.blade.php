    ...
    <div class="max-w-2xl">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            <div class="p-6 border-b border-gray-100">
                <h2 class="text-lg font-semibold text-gray-800">Create Menu Item</h2>
            </div>
            ...
            <form action="{{ route('admin.menus.store') }}" method="POST" class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="param" id="param" value="{{ old('param') }}"
                               class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                    </div>

                    <div>
                        <label for="menu_type" class="block text-sm font-medium text-gray-700 mb-1">Menu Type</label>
                        <input type="text" name="menu_type" id="menu_type" value="{{ old('menu_type') }}"
                               class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm"
                ...
                <div>
                    ...
                    <input type="text" name="stored_procedure" id="stored_procedure" value="{{ old('stored_procedure') }}"
                           class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm font-mono">
                </div>
                ...
                <div>
                    ...
                    <textarea name="params_json" id="params_json" rows="3"
                              class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm font-mono"
                              placeholder='{"key":"value"}'>{{ old('params_json') }}</textarea>
                    @error('params_json')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
